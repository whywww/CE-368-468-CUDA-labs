...
SM_VERSIONS   := sm_10 sm_11 sm_12 sm_13 sm_20 sm_30 sm_75

CUDA_INSTALL_PATH ?= /usr/local/cuda-11.0
CUDA_INCLUDE_PATH ?= $(CUDA_INSTALL_PATH)/targets/x86_64-linux/include
...
# Includes
INCLUDES  += -I. -I$(CUDA_INCLUDE_PATH) -I$(COMMONDIR)/inc -I$(SHAREDDIR)/inc
...
# Compiler-specific flags (by default, we always use sm_75), unless we use the SMVERSION template
...
# Default arch includes gencode for sm_75 and other archs from GENCODE_ARCH declared in the makefile
$(OBJDIR)/%.cu.o : $(SRCDIR)%.cu $(CU_DEPS)
	$(VERBOSE)$(NVCC) $(GENCODE_SM75) $(GENCODE_ARCH) $(NVCCFLAGS) $(SMVERSIONFLAGS) -o $@ -c $<

# Default arch includes gencode for sm_75, and other archs from GENCODE_ARCH declared in the makefile
$(CUBINDIR)/%.cubin : $(SRCDIR)%.cu cubindirectory
	$(VERBOSE)$(NVCC) $(GENCODE_SM75) $(GENCODE_ARCH) $(CUBIN_ARCH_FLAG) $(NVCCFLAGS) $(SMVERSIONFLAGS) -o $@ -cubin $<
...
#	$(VERBOSE)$(NVCC) -o $$@ -c $$< $(NVCCFLAGS)  $(1)
	$(VERBOSE)$(NVCC) -gencode=arch=compute_$(1),code=\"sm_$(1),compute_$(1)\" $(GENCODE_SM75) -o $$@ -c $$< $(NVCCFLAGS)
endef
